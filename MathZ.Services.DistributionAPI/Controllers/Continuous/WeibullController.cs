...
public class WeibullController(
    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the Weibull distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="weibullparams">The parameters for creating a Weibull distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] WeibullParams weibullparams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Weibull>(weibullparams);
    ...
    /// <summary>
    /// Computes the probability density of the Weibull distribution (PDF) at a given location (x), i.e., ∂P(X ≤ x)/∂x.
    /// </summary>
    /// <param name="weibullparams">The parameters for creating a Weibull distribution.</param>
    /// <param name="x">The location at which to compute the density.</param>
    /// <returns>The density at the specified location (x).</returns>
    [HttpPost("PDF/{x:double}")]
    public IActionResult PDF([FromBody] WeibullParams weibullparams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Weibull>(weibullparams);
    ...
    /// <summary>
    /// Computes the log probability density of the Weibull distribution (lnPDF) at a given location (x), i.e., ln(∂P(X ≤ x)/∂x).
    /// </summary>
    /// <param name="weibullparams">The parameters for creating a Weibull distribution.</param>
    /// <param name="x">The location at which to compute the log density.</param>
    /// <returns>The log density at the specified location (x).</returns>
    [HttpPost("PDFLn/{x:double}")]
    public IActionResult PDFLn([FromBody] WeibullParams weibullparams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Weibull>(weibullparams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the Weibull distribution.
    /// </summary>
    /// <param name="weibullparams">The parameters for creating a Weibull distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] WeibullParams weibullparams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<Weibull>(weibullparams);
