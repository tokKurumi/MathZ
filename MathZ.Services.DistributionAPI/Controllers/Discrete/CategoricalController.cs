...
public class CategoricalController(
    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the Categorical distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="categoricalParams">The parameters for creating a Categorical distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] CategoricalParams categoricalParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Categorical>(categoricalParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] CategoricalParams categoricalParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Categorical>(categoricalParams);
    ...
    public IActionResult PMFLn([FromBody] CategoricalParams categoricalParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Categorical>(categoricalParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the Categorical distribution.
    /// </summary>
    /// <param name="categoricalParams">The parameters for creating a Categorical distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] CategoricalParams categoricalParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<Categorical>(categoricalParams);
