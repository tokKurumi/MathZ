...
public class HypergeometricController(
    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the Hypergeometric distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="hypergeometricParams">The parameters for creating a Hypergeometric distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] HypergeometricParams hypergeometricParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Hypergeometric>(hypergeometricParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] HypergeometricParams hypergeometricParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Hypergeometric>(hypergeometricParams);
    ...
    public IActionResult PMFLn([FromBody] HypergeometricParams hypergeometricParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Hypergeometric>(hypergeometricParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the Hypergeometric distribution.
    /// </summary>
    /// <param name="hypergeometricParams">The parameters for creating a Hypergeometric distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] HypergeometricParams hypergeometricParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<Hypergeometric>(hypergeometricParams);
