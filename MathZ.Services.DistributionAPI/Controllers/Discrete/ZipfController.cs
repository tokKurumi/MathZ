    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the Zipf distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="zipfParams">The parameters for creating a Zipf distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] ZipfParams zipfParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Zipf>(zipfParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] ZipfParams zipfParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Zipf>(zipfParams);
    ...
    public IActionResult PMFLn([FromBody] ZipfParams zipfParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Zipf>(zipfParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the Zipf distribution.
    /// </summary>
    /// <param name="zipfParams">The parameters for creating a Zipf distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] ZipfParams zipfParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<Zipf>(zipfParams);
