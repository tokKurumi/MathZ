...
public class BernoulliController(
    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the Bernoulli distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="bernoulliParams">The parameters for creating a Bernoulli distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] BernoulliParams bernoulliParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Bernoulli>(bernoulliParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] BernoulliParams bernoulliParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Bernoulli>(bernoulliParams);
    ...
    public IActionResult PMFLn([FromBody] BernoulliParams bernoulliParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Bernoulli>(bernoulliParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the Bernoulli distribution.
    /// </summary>
    /// <param name="bernoulliParams">The parameters for creating a Bernoulli distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] BernoulliParams bernoulliParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<Bernoulli>(bernoulliParams);
