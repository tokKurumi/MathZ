...
public class BetaBinomialController(
    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the BetaBinomial distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="betaBinomialParams">The parameters for creating a BetaBinomial distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] BetaBinomialParams betaBinomialParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<BetaBinomial>(betaBinomialParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] BetaBinomialParams betaBinomialParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<BetaBinomial>(betaBinomialParams);
    ...
    public IActionResult PMFLn([FromBody] BetaBinomialParams betaBinomialParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<BetaBinomial>(betaBinomialParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the BetaBinomial distribution.
    /// </summary>
    /// <param name="betaBinomialParams">The parameters for creating a BetaBinomial distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] BetaBinomialParams betaBinomialParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<BetaBinomial>(betaBinomialParams);
