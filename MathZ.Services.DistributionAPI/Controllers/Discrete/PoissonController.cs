    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the Poisson distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="poissonParams">The parameters for creating a Poisson distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] PoissonParams poissonParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Poisson>(poissonParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] PoissonParams poissonParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Poisson>(poissonParams);
    ...
    public IActionResult PMFLn([FromBody] PoissonParams poissonParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Poisson>(poissonParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the Poisson distribution.
    /// </summary>
    /// <param name="poissonParams">The parameters for creating a Poisson distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] PoissonParams poissonParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<Poisson>(poissonParams);
