    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the Binomial distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="binomialParams">The parameters for creating a Binomial distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] BinomialParams binomialParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<Binomial>(binomialParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] BinomialParams binomialParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Binomial>(binomialParams);
    ...
    public IActionResult PMFLn([FromBody] BinomialParams binomialParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<Binomial>(binomialParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the Binomial distribution.
    /// </summary>
    /// <param name="binomialParams">The parameters for creating a Binomial distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] BinomialParams binomialParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<Binomial>(binomialParams);
