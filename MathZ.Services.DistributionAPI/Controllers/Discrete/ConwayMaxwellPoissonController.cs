...
public class ConwayMaxwellPoissonController(
    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the ConwayMaxwellPoisson distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="conwayMaxwellPoissonParams">The parameters for creating a ConwayMaxwellPoisson distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] ConwayMaxwellPoissonParams conwayMaxwellPoissonParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<ConwayMaxwellPoisson>(conwayMaxwellPoissonParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] ConwayMaxwellPoissonParams conwayMaxwellPoissonParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<ConwayMaxwellPoisson>(conwayMaxwellPoissonParams);
    ...
    public IActionResult PMFLn([FromBody] ConwayMaxwellPoissonParams conwayMaxwellPoissonParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<ConwayMaxwellPoisson>(conwayMaxwellPoissonParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the ConwayMaxwellPoisson distribution.
    /// </summary>
    /// <param name="conwayMaxwellPoissonParams">The parameters for creating a ConwayMaxwellPoisson distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] ConwayMaxwellPoissonParams conwayMaxwellPoissonParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<ConwayMaxwellPoisson>(conwayMaxwellPoissonParams);
