...
public class DiscreteUniformController(
    ...
    : ControllerBase
{
    ...
    /// <summary>
    /// Computes the cumulative distribution (CDF) of the DiscreteUniform distribution at a given location (x), i.e., P(X ≤ x).
    /// </summary>
    /// <param name="discreteUniformParams">The parameters for creating a DiscreteUniform distribution.</param>
    /// <param name="x">The location at which to compute the cumulative distribution function.</param>
    /// <returns>The cumulative distribution at the specified location (x).</returns>
    [HttpPost("CDF/{x:double}")]
    public IActionResult CDF([FromBody] DiscreteUniformParams discreteUniformParams, [FromRoute] double x)
    {
        try
        {
            var distribution = _mapper.Map<DiscreteUniform>(discreteUniformParams);
    ...
    [HttpPost("PMF/{k:int}")]
    public IActionResult PMF([FromBody] DiscreteUniformParams discreteUniformParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<DiscreteUniform>(discreteUniformParams);
    ...
    public IActionResult PMFLn([FromBody] DiscreteUniformParams discreteUniformParams, [FromRoute] int k)
    {
        try
        {
            var distribution = _mapper.Map<DiscreteUniform>(discreteUniformParams);
    ...
    /// <summary>
    /// Draws a sequence of random samples from the DiscreteUniform distribution.
    /// </summary>
    /// <param name="discreteUniformParams">The parameters for creating a DiscreteUniform distribution.</param>
    /// <param name="count">The number of samples to generate.</param>
    /// <returns>An infinite sequence of samples from the distribution.</returns>
    [HttpPost("Samples/{count:int}")]
    public IActionResult Samples([FromBody] DiscreteUniformParams discreteUniformParams, [FromRoute] int count)
    {
        try
        {
            var distribution = _mapper.Map<DiscreteUniform>(discreteUniformParams);
